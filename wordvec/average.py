...
def createVector(file):
  ...
  X = np.zeros((n, len(vocab_vec['wine'])))
  Y = []
  i = 0
  with open(file, 'r') as f:
    for line in f:
      ...
      array = np.zeros(300)
      for word in words:
        array = np.sum([array, np.array(vocab_vec[word]).astype(np.float)], axis = 0)
        count += 1
      X[i, :] = array/count
      Y.append(label)
      i += 1
...
if __name__ == '__main__':
  ...
  model = AdaBoostClassifier(learning_rate=0.3).fit(X_train, Y_train)
